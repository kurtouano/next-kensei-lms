// app/api/profile/banner/route.js
...
export async function POST(req) {
    try {
        ...
        const filename = `banner_${session.user.id}_${timestamp}.${fileExtension}`;

        const fileParams = {
            Bucket: process.env.AWS_S3_BUCKET_NAME,
            Key: `banners/${filename}`,
            ContentType: type,
            // Add metadata for banners
            Metadata: {
        ...
        const bannerUrl = `${process.env.AWS_CLOUDFRONT_DOMAIN_NAME}banners/${filename}`;
        ...
        console.error("Banner presigned URL error:", error);
        return NextResponse.json(
