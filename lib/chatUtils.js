...
async function getBroadcastFunctions() {
  if (!broadcastToChat) {
    try {
      ...
      console.error("Failed to import broadcast functions:", error)
    }
  }
...
export async function notifyNewMessage(chatId, message, excludeUserId = null) {
  try {
    console.log('notifyNewMessage called:', { chatId, messageType: message.type, messageId: message.id })
    const { broadcastToChat } = await getBroadcastFunctions()
    if (broadcastToChat) {
      console.log('Broadcasting message via broadcastToChat')
      ...
      console.log('broadcastToChat function not available')
    }
  } catch (error) {
