...
export async function notifyNewMessage(chatId, message, excludeUserId = null) {
  try {
    ...
    const success = await broadcastWithFallback(chatId, {
      type: "new_message",
      message,
    ...
    if (!success) {
      console.error("🚨 CRITICAL: All broadcast attempts failed")
      console.error("Chat ID:", chatId, "Message ID:", message.id)
      
      // Final fallback: Try again after a short delay
      setTimeout(async () => {
        console.log("🔄 Final retry attempt for message broadcast")
        await broadcastWithFallback(chatId, {
          type: "new_message",
          message,
          timestamp: new Date().toISOString(),
        }, excludeUserId)
