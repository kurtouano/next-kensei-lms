...
export async function notifyNewMessage(chatId, message, excludeUserId = null) {
  try {
    ...
    if (broadcastToChat) {
      ...
      console.error("🚨 CRITICAL: Broadcast function not available, message not sent via SSE")
      console.error("This means messages will not appear in real-time for other users!")
      console.error("Chat ID:", chatId, "Message ID:", message.id)
      
      // Try to re-import broadcast functions as a last resort
      setTimeout(async () => {
        try {
          const streamModule = await import("@/app/api/chats/stream/route")
          if (streamModule.broadcastToChat) {
            console.log("🔄 Retry: Broadcasting message via SSE")
