...
export async function createFriendChat(userId1, userId2) {
  try {
    ...
    // Check if direct chat already exists between these users (active or inactive)
    ...
    if (existingChat) {
      if (existingChat.isActive) {
        // Chat already exists and is active, return it
        return {
          success: true,
          chat: existingChat,
        ...
        console.log("Reactivating existing chat:", existingChat._id)
