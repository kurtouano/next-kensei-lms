  ...
  function middleware(req) {
    ...
    if (pathname.startsWith("/api/")) {
      ...
      if (
        ...
        pathname.match(
          /^\/api\/(courses|blogs)\/(create|[^\/]+\/(edit|delete))/
        )
      ) {
        if (!["instructor", "admin"].includes(userRole)) {
...
export const config = {
  matcher: [
     ...
     */
    "/((?!api/auth|_next/static|_next/image|favicon.ico|sitemap.xml|site.webmanifest|robots.txt|.*\\.png$|.*\\.jpg$|.*\\.jpeg$|.*\\.gif$|.*\\.svg$|.*\\.ico$|.*\\.webmanifest$).*)",
  ],
};
